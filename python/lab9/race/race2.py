...
# Initialize the new coin images
new_coin_images = ["ncoin.png", "wcoin.png"]
...
while True:
    ...
    if random.randint(0, 200) < 3:  # Probability for normal coins
        new_coin = Coin("coin.png", "normal")
        coins.add(new_coin)
        all_coin_sprites.add(new_coin)
    elif random.randint(0, 50) < 1:  # Probability for "ncoin" (adjust probability)
        new_coin = Coin("ncoin.png", "ncoin")
        coins.add(new_coin)
        all_coin_sprites.add(new_coin)
    elif random.randint(0, 50) < 1:  # Probability for "wcoin" (adjust probability)
        new_coin = Coin("wcoin.png", "wcoin")
        coins.add(new_coin)
        all_coin_sprites.add(new_coin)
    ...
    for coin in pygame.sprite.spritecollide(P1, coins, True):
        if coin.coin_type == "ncoin":
            SPEED += 1  # Increase speed if ncoin is collected
        elif coin.coin_type == "wcoin":
            SPEED -= 1  # Decrease speed if wcoin is collected
        COINS_COLLECTED += 1
